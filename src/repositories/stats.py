...
class Stats:
    ...
    def get_top_x_users(self, guild_id, member_ids: list, limit):
        q = '''
            ...
            sum(member_playing_duration) as member_playing_duration,
            ...
            sum(member_playing_duration) * lw.playing_weight  as playing_weighted,
            sum(message_count) * lw.message_weight + sum(emoji_count) * lw.reaction_weight + sum(voip_duration) * lw.voip_weight + sum(member_playing_duration) * lw.playing_weight as total_weighted
            from (
                ...
                0 as emoji_count,
                0 as voip_duration,
                sum(v.duration_time) as member_playing_duration
                from member_playing v 
                left join members m on m.id = v.member_id 
            ...
            group by q.member_id, lw.message_weight, lw.reaction_weight, lw.voip_weight, lw.playing_weight
            order by total_weighted desc
            limit {limit}
            ;
        '''.format(guild_id=guild_id, limit=limit, member_ids=','.join(map(str, member_ids)))
